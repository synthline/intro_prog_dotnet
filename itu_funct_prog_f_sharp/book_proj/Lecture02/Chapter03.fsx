...
let (+.) (x1, y1) (x2, y2): float * float = (x1 + x2, y1 + y2)

let (-.) v1 v2 = v1 +. -.v2

let ( *. ) x (x1, y1): float * float = (x * x1, x * y1)

let (&.) (x1, y1) (x2, y2): float = x1 * x2 + y1 * y2

let norm (x1: float, y1: float) = sqrt (x1 * x1 + y1 * y1)

let x: (int * int) list = [ (1, 2) ]
...
let solve1 (a, b, c) =
    if b * b - 4.0 * a * c < 0.0 || a = 0.0 then
        failwith "discriminant is negative or a=0.0"
    else
        ((-b + sqrt (b * b - 4.0 * a * c)) / (2.0 * a), (-b - sqrt (b * b - 4.0 * a * c)) / (2.0 * a))
